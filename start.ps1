...
# Layout:
#   DeploymentRoot\SetupScripts\start.ps1
#   DeploymentRoot\Konfigurationen\...
#   DeploymentRoot\Installationsprogramme\...
# Wir kopieren die *gesamte* DeploymentRoot nach C:\SetupScripts\Win11Deploy (ohne Rekursion).

$LocalDeploymentRoot = "C:\SetupScripts\Win11Deploy"
$LocalScriptRoot     = Join-Path $LocalDeploymentRoot "SetupScripts"

# Nur kopieren/relaunch, wenn wir nicht bereits lokal laufen
if ($DeploymentRoot -ne $LocalDeploymentRoot) {
    try {
        if (-not (Test-Path $LocalDeploymentRoot)) { New-Item -ItemType Directory -Path $LocalDeploymentRoot -Force | Out-Null }

        # Kopiere alle Items aus DeploymentRoot, aber ohne bereits existierendes Win11Deploy
        Get-ChildItem -LiteralPath $DeploymentRoot -Force | Where-Object { $_.Name -ne "Win11Deploy" } | ForEach-Object {
            Copy-Item -LiteralPath $_.FullName -Destination $LocalDeploymentRoot -Recurse -Force -ErrorAction Stop
        }

        # Relaunch lokal (im SetupScripts-Unterordner)
        $LocalStart = Join-Path $LocalScriptRoot "start.ps1"
...
# Nach Relaunch: Pfade auf lokal setzen
$DeploymentRoot = $LocalDeploymentRoot
